...
pull the top ~200 targets by votes, buy one cheap draw from our **mtok.market** house seller to ask
which of the currently *displayed* ones are vulgar, and append literal blocklist entries between the
`audit:begin`/`audit:end` markers in `filter.ts`. The model judges WHAT; the script decides HOW
